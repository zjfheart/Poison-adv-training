...
parser.add_argument('--data_mode', type=str, default='mix')
...
if args.data_mode == 'mix':
    train_loader = get_poisoned_loader(
        'cifar10', batch_size=128, root='../data', train=True,
        noise_path='../poisoning/exp_data/cifar10/patch_em_notrans_loc4/ss35_ps0.03/em-fin-def-noise.pkl',
        noise_rate=1, poisoned_indices_path=None,
        mask_path='../poisoning/exp_data/cifar10/patch_em_notrans_loc4/ss35_ps0.03/em-fin-def-mask.pkl')
elif args.data_mode == 'clear':
    train_loader = get_clear_loader(
        'cifar10', batch_size=128, root='../data', train=True,
        poisoned_indices_path=None)
else:
    raise NotImplementedError

test_loader = get_poisoned_loader(
    'cifar10', batch_size=128, root='../data', train=False)
...
for epoch in range(start_epoch, args.epochs):
    ...
    loss, test_pgd10_acc = attack.eval_robust(model, test_loader, perturb_steps=10, epsilon=0.031, step_size=0.031 / 4,loss_fn="cent", category="Madry", rand_init=True)
