...
class PoisonedDataset(Dataset):
    def __init__(self, dataset, noise, mask=None, fitr=None):
        assert isinstance(dataset, Dataset)
        self.loader    = dataset.loader
        self.classes   = dataset.classes
        self.samples   = dataset.samples
    ...
    def __getitem__(self, idx):
        ...
        if self.mask is not None:
            x = (np.asarray(x, dtype=np.int16) * (1 - self.mask.astype(np.int16))
                 + self.noise[idx].astype(np.int16) * self.mask.astype(np.int16)).clip(0, 255).astype(np.uint8)
        else:
...
def get_poisoned_loader(
    ...
    if noise_path is not None:
        ...
        if type == 'patch':
            target_set = PoisonedDataset(target_set, noise, mask)
        elif type == 'rem':
            target_set = PoisonedDataset(target_set, noise)
