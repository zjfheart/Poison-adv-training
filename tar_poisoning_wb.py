...
parser.add_argument('--paugment', action='store_true')
...
parser.add_argument('--clean-threat', default=True, type=bool)
...
for trial in range(args.restarts):
    # Init poison delta
    poison_delta = data.initialize_poison()

    # For momentum optimizers, do common init operations
    if args.attackoptim in ['Adam', 'signAdam']:
        att_optimizer = torch.optim.Adam([poison_delta], lr=args.tau, weight_decay=0)
    ...
    for iter in range(args.attackiter):
        ...
        for batch, (inputs, labels, ids) in enumerate(data.poisonloader):
            ...
            delta_slice = poison_delta[batch_slice].detach().to(**setup)
            delta_slice.requires_grad_()
            poison_images = inputs.clone()
            inputs_ = inputs + delta_slice

            if args.paugment:
                inputs_ = data.augment(inputs_)

            # Compute loss
            loss, prediction = similarity_loss(args.loss, model.model, model.criterion, inputs_, labels,
            ...
            # Update Step
            poison_delta.grad[batch_slice] = delta_slice.grad.detach().to(device=torch.device('cpu'))
            poison_bounds[batch_slice] = poison_images.detach().to(device=torch.device('cpu'))

        # For the momentum optimizers, use optimizer.step() for the update
        if args.attackoptim in ['signAdam']:
        ...
        with torch.no_grad():
            # Projection Step
            poison_delta.data = torch.max(torch.min(poison_delta, args.eps / ds / 255), -args.eps / ds / 255)
            poison_delta.data = torch.max(torch.min(poison_delta, (1 - dm) / ds - poison_bounds), -dm / ds - poison_bounds)
